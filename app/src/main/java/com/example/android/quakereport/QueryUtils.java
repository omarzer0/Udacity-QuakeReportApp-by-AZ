...
//                    "{\"type\":\"Feature\",\"properties\":{\"mag\":6,\"place\":\"Pacific-Antarctic Ridge\",\"time\":1451986454620,\"updated\":1459202978040,\"tz\":-540,\"url\":\"http://earthquake.usgs.gov/earthquakes/eventpage/us10004bgk\",\"detail\":\"http://earthquake.usgs.gov/fdsnws/event/1/query?eventid=us10004bgk&format=geojson\",\"felt\":0,\"cdi\":1,\"mmi\":0,\"alert\":\"green\",\"status\":\"reviewed\",\"tsunami\":0,\"sig\":554,\"net\":\"us\",\"code\":\"10004bgk\",\"ids\":\",us10004bgk,gcmt20160105093415,\",\"sources\":\",us,gcmt,\",\"types\":\",cap,dyfi,geoserve,losspager,moment-tensor,nearby-cities,origin,phase-data,shakemap,\",\"nst\":null,\"dmin\":30.75,\"rms\":0.67,\"gap\":71,\"magType\":\"mww\",\"type\":\"earthquake\",\"title\":\"M 6.0 - Pacific-Antarctic Ridge\"},\"geometry\":{\"type\":\"Point\",\"coordinates\":[-136.2603,-54.2906,10]},\"id\":\"us10004bgk\"}],\"bbox\":[-153.4051,-54.2906,10,158.5463,59.6363,582.56]}";
    ...
    public static List<Quake> fetchEarthquakeData(String urlRequest) {
        // 1-Create URL object
        URL url = createURL(urlRequest);
        // 2-Perform HTTP request to the URL and receive a JSON response back
        String jsonResponed = null;
        try {
            jsonResponed = makeHTTPRequest(url);
        } catch (IOException e) {
            e.printStackTrace();
        }
        // 3-Extract relevant fields from the JSON response and create an {@link Quake} object
        List<Quake> quakeArrayList = extractFeatureFromJson(jsonResponed);
        // 4-Return the {@link Event}
    ...
    public static List<Quake> extractFeatureFromJson(String jsonResponed) {
        // If the JSON string is empty or null, then return early.
        if (TextUtils.isEmpty(jsonResponed)) {
            return null;
        }


        // Create an empty ArrayList that we can start adding earthquakes to
        List<Quake> earthquakesList = new ArrayList<>();

        // Try to parse the SAMPLE_JSON_RESPONSE. If there's a problem with the way the JSON
        // is formatted, a JSONException exception object will be thrown.
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
        try {
            JSONObject root = new JSONObject(jsonResponed);
            ...
            for (int i = 0; i < features.length(); i++) {
                ...
                earthquakesList.add(new Quake(mag, place, time, url));
            }
    ...
    private static String makeHTTPRequest(URL url) throws IOException {
        ...
        try {
            ...
            if (urlConnection.getResponseCode() == 200) {
                ...
                //use helper method to get the inputStream
                jsonResponed = readFromStream(inputStream);
            } else {
                Log.e("error", "Connection failed." + urlConnection.getResponseCode());
            }
